...
var bundles = map[Lang]bundle{
	LangZH: {
		...
		legend: map[scanner.Category]string{
			scanner.ARG:     "问题类型 - ARG: map赋值参数",
			scanner.REF:     "问题类型 - REF: map下标引用",
			scanner.MODULE:  "问题类型 - MODULE: 受影响模块引用",
			scanner.PRESENT: "问题类型 - PRESENT: 受影响资源存在",
		},
	...
	LangEN: {
		...
		legend: map[scanner.Category]string{
			scanner.ARG:     "Issue type - ARG: map-assign argument",
			scanner.REF:     "Issue type - REF: map-index reference",
			scanner.MODULE:  "Issue type - MODULE: known-affected module",
			scanner.PRESENT: "Issue type - PRESENT: affected resource present",
		},
